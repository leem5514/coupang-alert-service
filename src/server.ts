...
function dashboardData() {
  ...
  const items = getAllActiveWatchItems().map(item => ({
    ...item,
    history: getRecentHistory(item.id, 12),
    schedule: statuses.get(item.id) ?? null,
  }));
  return { items, api: getApiRateLimitStatus(), updatedAt: new Date().toISOString() };
}

async function handleApi(req: IncomingMessage, res: ServerResponse, pathname: string): Promise<boolean> {
  if (req.method === 'GET' && pathname === '/api/dashboard') {
    json(res, 200, dashboardData());
    return true;
  }
  ...
  if (req.method === 'POST' && pathname === '/api/watch-items') {
    ...
    if (keyword.length < 2 || keyword.length > 80) {
      json(res, 400, { message: '상품 키워드는 2~80자로 입력해 주세요.' });
      return true;
    }
    if (!Number.isInteger(targetPrice) || targetPrice <= 0 || targetPrice > 1_000_000_000) {
      json(res, 400, { message: '올바른 목표 가격을 입력해 주세요.' });
      return true;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      json(res, 400, { message: '올바른 이메일 주소를 입력해 주세요.' });
      return true;
    }
...
async function serveStatic(res: ServerResponse, pathname: string): Promise<void> {
  ...
  if (!filePath.startsWith(PUBLIC_DIR)) {
    json(res, 403, { message: '허용되지 않은 경로입니다.' });
    return;
  }
  ...
  try {
    ...
    json(res, 404, { message: '페이지를 찾을 수 없습니다.' });
  }
}
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (!handled) json(res, 404, { message: 'API를 찾을 수 없습니다.' });
      return;
    }
...
startScheduler();
server.listen(PORT, () => {
  console.log(`쿠팡 가격 알림 대시보드: http://localhost:${PORT}`);
});
